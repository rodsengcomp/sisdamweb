...
$(document).ready(function() {
    ...
    $('#list-memo-2017').DataTable({responsive: {details: {display: $.fn.dataTable.Responsive.display.modal({header: function (row) {var data = row.data();
        ...
        "language": {
            ...
            "oAria": {"sSortAscending": "Ordenar colunas de forma ascendente","sPrevious": "Ordenar colunas de forma descendente"} },
        dom: "lBfrtip",processing: true, serverside: true, ajax: 'form-system/list/proc-list-system/list-memo/list-memo-2017.php',
        "lengthMenu": [[6, 10, 25, 50, -1], [6, 10, 25, 50, "Todos"]], "aaSorting": [ 0, 'desc' ],
        "aoColumnDefs": [{"aTargets": [0] },
            {
                "aTargets": [1]// o numero 6 é o nº da coluna
            } ],
        buttons: [ {extend:'excel',title:'Memorandos 2017',header: 'Memorandos 2017',filename:'Memorandos 2017',className: 'btn btn-success',text:'<span class="fa fa-file-excel-o"></span>' },
            {extend: 'pdfHtml5',exportOptions: {columns: ':visible'},title:'Memorandos 2017',header: 'Memorandos 2017',filename:'Memorandos 2017',orientation: 'landscape',pageSize: 'LEGAL',className: 'btn btn-danger',text:'<span class="fa fa-file-pdf-o"></span>'},
            {extend:'print', exportOptions: {columns: ':visible'},title:'Memorandos 2017',header: 'Memorandos 2017',filename:'Memorandos 2017',className: 'btn btn-default',text:'<span class="fa fa-print"></span>'},
            {extend:'colvis',titleAttr: 'Select Colunas',className: 'btn btn-info',text:'<span class="fa fa-list"></span>'} ]
    });

    $('#list-memo-2018').DataTable({responsive: {details: {display: $.fn.dataTable.Responsive.display.modal({header: function (row) {var data = row.data();
                        return 'DETALHES DO ' +data[3]+ '&nbspNº&nbsp:&nbsp'  + data[1];}}),renderer: function ( api, rowIdx, columns ) {var data = $.map( columns, function ( col, i ) {
                    return '<tr>'+'<td>'+col.title+':'+'</td> '+'<td>'+col.data+'</td>'+'</tr>';} ).join('');
                    return $('<table/>').append( data ); } } },
        "language": {
            ...
            "sSearch": "Pesquisar","oPaginate": {"sNext": "Próximo","sPrevious": "Anterior","sFirst": "Primeiro","sLast": "Último"},
            "oAria": {"sSortAscending": "Ordenar colunas de forma ascendente","sPrevious": "Ordenar colunas de forma descendente"} },
        dom: "lBfrtip",processing: true, serverside: true, ajax: 'form-system/list/proc-list-system/list-memo/list-memo-2018.php',
        "lengthMenu": [[6, 10, 25, 50, -1], [6, 10, 25, 50, "Todos"]], "aaSorting": [ 0, 'desc' ],
        "aoColumnDefs": [{"aTargets": [0] },
            {
                "aTargets": [1]// o numero 6 é o nº da coluna
            } ],
            buttons: [ {extend:'excel',title:'Memorandos 2018',header: 'Memorandos 2018',filename:'Memorandos 2018',className: 'btn btn-success',text:'<span class="fa fa-file-excel-o"></span>' },
            {extend: 'pdfHtml5',exportOptions: {columns: ':visible'},title:'Memorandos 2018',header: 'Memorandos 2018',filename:'Memorandos 2018',orientation: 'landscape',pageSize: 'LEGAL',className: 'btn btn-danger',text:'<span class="fa fa-file-pdf-o"></span>'},
            {extend:'print', exportOptions: {columns: ':visible'},title:'Memorandos 2018',header: 'Memorandos 2018',filename:'Memorandos 2018',className: 'btn btn-default',text:'<span class="fa fa-print"></span>'},
            {extend:'colvis',titleAttr: 'Select Colunas',className: 'btn btn-info',text:'<span class="fa fa-list"></span>'} ]
    });
    ...
    var table = $('#list-adm-sanitaria').DataTable({responsive: {details: {display: $.fn.dataTable.Responsive.display.modal({header: function (row) {var data = row.data();
        ...
        "aoColumnDefs": [
            ...
            {"bVisible": false,"aTargets": [18]},
            {
                "aTargets": [1], // o numero 6 é o nº da coluna
                "mRender": function (data, type, full) { //aqui é uma funçãozinha para pegar os ids
                    return '<a href="suvisjt.php?pag=edit-prot-sanitaria&id=' + full[0] + '" role="button" class="btn btn-warning rounded-circle"><span class="glyphicon glyphicon-pencil"></a>';
                }
            }
        ],
