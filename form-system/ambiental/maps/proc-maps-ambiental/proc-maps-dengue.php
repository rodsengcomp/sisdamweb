...
(CASE
WHEN CLASSI_FIN>9 AND Resultado_IgM_Focus='Reagente ' THEN 'Positivo'
WHEN CLASSI_FIN>9 AND Resultado_IgM_Panbio='Reagente ' THEN 'Positivo'
WHEN CLASSI_FIN>9 AND Resultado_NS1='Reagente ' THEN 'Positivo'
WHEN CLASSI_FIN>9 AND ResultadoTr='Reagente' THEN 'Positivo'
-- WHEN Resultado_IgM_Focus='Não Reagente' THEN 'Negativo' -- sem efeito no mapa
-- WHEN Resultado_IgM_Panbio='Não Reagen' THEN 'Negativo' -- sem efeito no mapa
-- Agora os amarelos = Exame
WHEN ResultadoTr='Exame Nao Realizado' AND Resultado_IgM_Focus IS NULL AND Resultado_NS1 IS NULL THEN 'Exame'
WHEN ResultadoTr='Exame Nao Realizado' AND Resultado_IgM_Focus=' ' AND Resultado_NS1=' ' THEN 'Exame'
WHEN ResultadoTr='Exame Nao Realizado' AND Resultado_IgM_Focus='Coleta Inadequada ' AND Resultado_NS1=' ' THEN 'Inconclusivo'
WHEN ResultadoTr='Exame Nao Realizado' AND Resultado_IgM_Focus='Inconclusivo ' AND Resultado_NS1=' ' THEN 'Inconclusivo'
...
resultado_ccz.Resultado_IgM_Focus, resultado_ccz.Coleta,resultado_ccz.Resultado_IgM_Panbio,resultado_ccz.Resultado_NS1,resultado_ccz.`Data Resultado` ,
ruas.ruagoogle,ruas.log,ruas.rua,ruas.id,
dengnet.NU_NOTIFIC, dengnet.CLASSI_FIN, dengnet.NM_PACIENT, dengnet.DT_SIN_PRI, dengnet.DT_NOTIFIC
...
while ($row_markers = mysqli_fetch_assoc($resultado_markers)) {
    ...
    echo 'dtnot="'. $row_markers['DT_NOTIFIC'].'" ';
    echo 'teste="'. $row_markers['ResultadoTr'].'" ';
    echo 'coleta="'. $row_markers['Coleta'].'" ';
    echo 'dtres="'. $row_markers['Data Resultado'].'" ';
    echo 'igm="'. $row_markers['Resultado_IgM_Focus'].'" ';
    echo 'igmpan="'. $row_markers['Resultado_IgM_Panbio'].'" ';
