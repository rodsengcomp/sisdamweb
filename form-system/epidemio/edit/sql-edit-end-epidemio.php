...
$sql_end_null = $conectar->query("SELECT $agravo_sinan_sql.NU_NOTIFIC, $agravo_sinan_sql.DT_NOTIFIC, $agravo_sinan_sql.NM_PACIENT, $agravo_sinan_sql.ID_DISTRIT, $agravo_sinan_sql.ID_BAIRRO, $agravo_sinan_sql.NM_BAIRRO, $agravo_sinan_sql.ID_LOGRADO, $agravo_sinan_sql.NM_LOGRADO, $agravo_sinan_sql.NU_NUMERO, $agravo_sinan_sql.NM_COMPLEM, $agravo_sinan_sql.ID_UNIDADE, $agravo_tabela_sql.DataEntrada
...
// Consulta sql para redirecionamento após findar os endereços para atualizar
    if ($_SESSION['usuarioNivelAcesso'] <> '') {
        if ($sql_end_null->num_rows == 0) {
            $_SESSION['msgsuccess'] = "<div class='alert alert-success text-center'><strong>NÃO EXISTEM MAIS CASOS DE SARAMPO PARA ATUALIZAR !!!</strong></div>";
            if ($agravo_buttons == 'sarampo') header("Location: suvisjt.php?pag=list-sinan-ial-sarampo");
