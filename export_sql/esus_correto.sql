...
insert into esus
SELECT esus_aline_atual_final.`PROT_ESUS` AS `Número da Notificação`, esus_aline_atual_final.`DT_NOTIFIC` AS `Data da Notificação`, esus_aline_atual_final.`TEL` AS `Telefone de Contato`,
       UCASE(esus_aline_atual_final_mes.`NOME`) AS `Nome Completo`, CONCAT(RIGHT(esus_aline_atual_final.DT_NASC,4),"-",MID(esus_aline_atual_final.DT_NASC,4,2),"-", (LEFT(esus_aline_atual_final.DT_NASC,2))) AS `Data de Nascimento`,
       esus_aline_atual_final.SEXO AS `Sexo`, esus_aline_atual_final_mes.`DT_SINT`, esus_aline_atual_final_mes.`SE_DT_SINT`, esus_aline_atual_final.`TEL` AS `Telefone Celular`,
       CONCAT(LEFT(e esus_aline_atual_final.CEP,2), MID( esus_aline_atual_final.CEP,4)) AS `cep`,
       ruas_jacana_cep_esus.log AS `Logradouro`,ruas_jacana_cep_esus.bairro AS Bairro, esus_aline_atual_final_mes.NU_NM AS `Número (ou SN para Sem Número)`,esus_aline_atual_final_mes.NM_COMP
       AS Complemento, ruas_jacana_cep_esus.ubs AS Ubs, "UVIS JACANA-TREMEMBE" AS UVIS, ruas_jacana_cep_esus.latitude AS LATSIRGAS,ruas_jacana_cep_esus.longitude AS LONSIRGAS,
       ruas_jacana_cep_esus.da AS Da, esus_aline_atual_final_mes.`NT_CNES` AS CNES, esus_aline_atual_final_mes.UN_NOTIFIC AS unidade_de_atendimento
FROM esus_aline_atual_final LEFT JOIN ruas_jacana_cep_esus ON esus_aline.CEP = ruas_jacana_cep_esus.cep;
-- WHERE esus_total.`Evolução Caso`<>'Cancelado';

-- deletandos as linnhas duplicadas da tabela "esus_total"
DELETE a FROM `esus` AS a, `esus` AS b WHERE a.
PROT_ESUS=b.PROT_ESUS AND a.id < b.id;

-- após todas as tratativas com a tabela "esus_total" apagando a coluna "id" da tabela
ALTER TABLE `esus` DROP `id`;
...
SELECT "" AS id, "0000000" AS sinan, esus.`Número da Notificação` AS `protocolo`, esus.`Data da Notificação` AS datanot,
       ...
       "" AS log, esus.Logradouro AS rua, "" AS bairro, esus.Ubs AS localvd, "UVIS JACANA-TREMEMBE" AS suvis,
       "SAO PAULO" AS cidade, "" AS idrua, "" AS dataobito,NOW() AS criado, esus.LATSIRGAS AS latsv2, esus.LONSIRGAS AS longsv2,
       "sisdamweb D791749" AS usuariocad, "" AS alterado, "" AS usuarioalt, "" AS ocorrencia, "suvis" AS tipo, '0' AS lixeira
