...
-- LOAD DATA INFILE 'C:/csv/esus/esus_27072021/esus_julho_27072021.csv'
LOAD DATA INFILE 'c:/csv/esus/esus_3108/esus_covisa_mar_20.csv'
...
SELECT DISTINCT esus_total.`Nome Completo` AS NOME, esus_total.`Número da Notificação` AS `PROT_ESUS`, esus_total.`Data da Notificação` AS `DT_NOTIFIC`,esus_total.`Telefone de Contato` AS TEL,
                ...
                esus_total.`Descrição do Sintoma` AS DESC_SINT, esus_total.`Data de Nascimento` AS DT_NASC, esus_total.`Data de encerramento` AS DT_ENC,
                esus_total.`Notificante CNES` AS NT_CNES, esus_aline.`UN_NOTIFIC`, esus_total.`Notificante Email` AS NT_EMAIL, esus_total.`Notificante Nome Completo` AS NT_NOME
FROM esus_aline RIGHT JOIN (esus_total LEFT JOIN ruas_jacana_cep_esus ON esus_total.CEP = ruas_jacana_cep_esus.cep)
                           ON esus_aline.`PROT_ESUS` = esus_total.`Número da Notificação` WHERE ((esus_aline.NOME) Is Null) AND esus_total.`Evolução Caso`<>'CANCELADO' ;
-- (((UCase(esus_total.`Evolução Caso`))="" Or (UCase(esus_total.`Evolução Caso`)) Is Null) AND ((esus_aline.NOME) Is Null));
-- AND ((ruas_jacana_cep_esus.cep) In (SELECT `cep` FROM `ruas_jacana_cep_esus` As Tmp GROUP BY `cep` HAVING Count(*)=1 )));
...
-- inserindo o campo "id" na tabela "esus_total"
ALTER TABLE `esus_aline_atual_final_mes` ADD `id` INT NOT NULL AUTO_INCREMENT FIRST, ADD PRIMARY KEY (`id`);
-- deletandos as linhas duplicadas da tabela "esus_total"
DELETE a FROM `esus_aline_atual_final_mes` AS a, `esus` AS b WHERE a.
esus_aline_atual_final_mes.`PROT_ESUS`=b.`PROT_ESUS` AND a.id < b.id;

-- após todas as tratativas com a tabela "esus_total" apagando a coluna "id" da tabela
ALTER TABLE `esus_aline_atual_final_mes` DROP `id`;
