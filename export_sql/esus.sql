...
esus_total.`Notificante CNES` AS NT_CNES, esus_total.`Notificante Email` AS NT_EMAIL, esus_total.`Notificante Nome Completo` AS NT_NOME
...
end) AS `LOCAL_ATEND`, esus_total.`Tipo de Teste` AS `TIP_TEST`, esus_total.`Data do Teste (PCR/Rápidos)` AS `DT_PCR_RAP`,
UCASE(esus_total.`Resultado (PCR/Rápidos)`) AS `RES_PCR_RAP_ESUS`,
esus_total.`Data do Teste (Sorologico)` AS `DT_SORO_ESUS`,UCASE(esus_total.`Resultado IgM`) AS `RES_IGM_ESUS`,
UCASE(esus_total.`Resultado IgG`) AS `RES_IGG_ESUS`, UCASE(esus_total.`Resultado IgA`) AS `RES_IGA_ESUS`,
UCASE(esus_total.`Resultado Totais`) AS `RES_TOT_ESUS`,esus_aline.`DT_COL_UVIS`, esus_aline.`RES_UVIS`,
...
esus_aline_atual.`UBS_ABRANG`, esus_aline_atual.`LOCAL_ATEND`, esus_aline_atual.`TIP_TEST`, esus_aline_atual.`DT_PCR_RAP`,
...
insert into esus
SELECT esus_total.`Número da Notificação`, esus_total.`Data da Notificação`, esus_total.`Telefone de Contato`, UCASE(esus_aline_atual_final.`NOME`) AS `Nome Completo`,
...
esus_aline_atual_final.`SE_DT_SINT`, esus_total.`Telefone Celular`, CONCAT(LEFT(esus_total.CEP,2), MID(esus_total.CEP,4)) AS `cep`,
