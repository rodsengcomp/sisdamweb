...
if ($btnCadUsuario) {
    ...
    if (!$erro) {
        ...
        $result_usuario = "INSERT INTO usuarios (nome, email, login, senha, nivel_acesso_id, status, acessotid, usuariocad, criado) VALUES (
						'" . $dados['nome'] . "',
						'" . $dados['email'] . "',
						'" . $dados['login'] . "',
						'" . $dados['senha'] . "',
        ...
        if (mysqli_insert_id($conectar)) {
            $_SESSION['msgcad'] = "<div class='alert alert-success text-center'>Usuário : " . $dados['login'] . " <br>Cadastrado com sucesso!</div>";
        } else {
            $_SESSION['msg'] = "<div class='alert alert-danger text-center'>Erro ao cadastrar o usuário : " . $dados['login'] . "!</div>";
        }
    }
