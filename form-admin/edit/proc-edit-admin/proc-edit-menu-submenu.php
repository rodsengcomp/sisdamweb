...
if ($btnEditMenuSubMenu) {
    ...
    if (in_array('', $dados)) {
        ...
        $result_menu_submenu = "SELECT id FROM menu_sub WHERE id='" . $dados['id_menu_sub'] . "'";
        ...
        if (($resultado_menu_submenu) AND ($resultado_menu_submenu->num_rows != 1)) {
            $erro = true;
            $_SESSION['msgerro'] = "<div class='alert alert-danger text-center'>O SuMenu id : " . $dados['id_menu_sub'] . " <br>Não existe !</div>";
        }
    ...
    //var_dump($dados);
    if (!$erro) {
        $result_menu_submenu = "UPDATE menu_sub_sub set id_menu_sub='" . $dados['id_menu_sub'] . "', nome='" . $dados['nome'] . "', pag='" . $dados['pag'] . "', usuarioalt='$usuariologin', alterado= NOW() WHERE id='" . $dados['id'] . "'";
        $resultado_menu_submenu = mysqli_query($conectar, $result_menu_submenu);
        if (mysqli_thread_id($conectar)) {
            $_SESSION['msgedit'] = "<div class='alert alert-success text-center'><strong>Menu SubMenu</strong> : " . $dados['nome'] . " - <strong>Pag</strong> : " . $dados['pag'] . " <br><strong>Atualizado com sucesso!</strong></div>";
            header("Location: admin.php?pag=list-menu-submenu");
        } else {
